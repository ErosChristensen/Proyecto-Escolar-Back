...
function buildUpdateSet(body) {
  const allowed = ["titulo", "subtitulo", "descripcion", "fecha", "imagen1", "imagen2", "imagen3" , "imagen4"];
  const fields = [];
  const values = [];
  ...
  try {
    ...
    if (!q || q.length < 3) {
      const [rows] = await pool.query(
        `SELECT SQL_CALC_FOUND_ROWS id_noticias, titulo,subtitulo, descripcion, fecha, imagen1, imagen2, imagen3 , imagen4
           FROM noticias
    ...
    let [items] = await pool.query(
      `SELECT id_noticias, titulo, subtitulo ,descripcion, fecha, imagen1, imagen2, imagen3 , imagen4
         FROM noticias
